...
coord2vert (dimX, dimY, dimZ) (x,y,z) = if (x >= dimX || y >= dimY || z >= dimZ)
  then -1
  else
    case (x,y,z) of
      (0,0,0) -> 0
...
neighbourhood (dimX, dimY, dimZ) coord@(x,y,z) = Prelude.filter edgeCase [ (x+i, y+j, z+k) | i <- [-1,0,1], j <- [-1,0,1], k <- [-1,0,1] ]
    where
      edgeCase = \v@(x',y',z') -> coord /= v && (x' >= 0 && y' >= 0 && z' >= 0 && x' < dimX && y' < dimY && z' < dimZ)
...
joinTree vertices adjacent = tree $ foreach vertices (initJoin vertices) $ \jI v ->
    foreach (adjacent v) (mark v jI) $ \jI n ->
      ...
      let components   = disj jI
          ...
          compV        = comp v
          compN        = comp n
          union        = S.union compV compN
          replaceUnion = \_ -> union
          u            = lowest compN
      in if ( n `S.member` (seen jI) ) && (compV /= compN)
         then
               jI {
                  disj = I.adjust replaceUnion n components,
                  tree = (u,v) : (tree jI),
...
tree1 = joinTree [0..length vert1 -1] (mesh ds1)

dim2  = (5,5,5)
vert2 = [1..5^3]
ds2   = dataset dim2 (neighbourhood dim2) vert2
tree2 = joinTree [0..length vert2-1] (mesh ds2)

dim3  = (2,2,1)
vert3 = [1..4]
ds3   = dataset dim3 (neighbourhood dim3) vert3
tree3 = joinTree [0..length vert3 - 1] (mesh ds3)

dim4  = (2,1,1)
vert4 = [1,2]
ds4   = dataset dim4 (neighbourhood dim4) vert4
tree4 = joinTree [0..length vert4 - 1] (mesh ds4)
